...
exports.getSpotPrices = async function (
  ethBalanceWei,
  exchangeTokenBalanceWei,
  display,
) {
...
exports.getEffectivePrices = async function (
  web3,
  ethSpendWei,
  exchangeEthBalance,
  ...
  var numerator = ethSpendWeiBNWithFee
    .multipliedBy(exchangeTokenBalanceBN)
    .dividedBy(weiBN) // For calculating price - see UniSwap exchange contract
  ...
  var effectivePriceBN = ethSpendWeiBN
    .dividedBy(tokensToBuyBN.multipliedBy(weiBN))
    .precision(18)

  if (display) {
    console.log(
      'Spend: ' + web3.utils.fromWei(ethSpendWei.toString(10), 'ether'),
    )
    console.log(
      'Tokens To Buy: ' +
        web3.utils.fromWei(tokensToBuyWei.toString(10), 'ether'),
    )
  ...
  return {
    ethSpendWei: ethSpendWei,
    tokensToBuyWeiBN: tokensToBuyWeiBN,
    effectivePriceBN: effectivePriceBN,
  }
}
...
exports.sendTransactionWithValue = async function (
  web3,
  tx,
  Account,
  PrivateKey,
